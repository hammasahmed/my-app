...
export default function Dashboard1() {
  ...
  const toggleHide = async (youtubeId: string) => {
    if (hiddenVideos.has(youtubeId)) {
      await axios
        .delete(`${BASE_URL}/api/hidden-videos/${youtubeId}`)
        .catch(() => {});
      ...
      await axios
        .post(`${BASE_URL}/api/hidden-videos`, { youtubeId })
        .catch(() => {});
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () =>
      setTestimonialForm((p) => ({ ...p, src: reader.result as string }));
  ...
  const handleAdImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader();
    reader.onload = () =>
      setAdForm((p) => ({ ...p, src: reader.result as string }));
    reader.readAsDataURL(file);
  };
  ...
  const handleGalleryImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader();
    reader.onload = () =>
      setGalleryForm((p) => ({ ...p, src: reader.result as string }));
    reader.readAsDataURL(file);
  };
  ...
  return (
    <div
      className={`min-h-svh w-full ${dark ? "bg-[#141414] text-zinc-100" : "bg-zinc-50 text-zinc-900"}`}
    >
      ...
      >
        ...
        />
        <SidebarInset
          className={`flex min-h-svh flex-col overflow-x-hidden ${dark ? "bg-transparent" : "bg-zinc-50"}`}
        >
          <header
            className={`sticky top-0 z-20 flex min-h-16 shrink-0 flex-wrap items-center gap-3 border-b px-3 py-3 backdrop-blur-md sm:gap-4 sm:px-4 md:min-h-14 md:flex-nowrap md:justify-between ${dark ? "border-white/10 bg-zinc-950/80" : "border-zinc-200 bg-white"}`}
          >
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
              <SidebarTrigger
                className={`-ml-0.5 shrink-0 border ${dark ? "border-white/15 bg-zinc-900/80 text-zinc-100 hover:bg-zinc-800 hover:text-white" : "border-zinc-200 bg-white text-zinc-800 hover:bg-zinc-100"}`}
              />
              <Separator
              ...
              <Breadcrumb className="min-w-0">
                <BreadcrumbList className="flex-wrap gap-x-2 gap-y-1">
                  ...
                  <BreadcrumbSeparator
                    className={`hidden sm:block ${dark ? "[&>svg]:text-zinc-500" : "[&>svg]:text-zinc-400"}`}
                  />
                  <BreadcrumbItem>
                    <BreadcrumbPage
                      className={`truncate text-xs font-medium ${dark ? "text-zinc-100" : "text-zinc-800"}`}
                    >
                      Dashboard
                    </BreadcrumbPage>
                  </BreadcrumbItem>
          ...
          <div className="flex flex-1 flex-col gap-6 p-4 pt-5 sm:p-6">
            <div className="space-y-2 text-center sm:text-left">
              ...
              <h1
                className={`text-2xl font-bold tracking-tight sm:text-3xl ${dark ? "text-zinc-50" : "text-zinc-900"}`}
              >
                Dashboard
              </h1>
            </div>
            ...
            <div
              id="videos-section"
              className={`flex min-h-[min(60vh,520px)] flex-1 flex-col rounded-3xl border p-6 backdrop-blur-sm md:min-h-0 ${dark ? "border-white/10 bg-zinc-900/40 shadow-[0_30px_120px_rgba(0,0,0,0.5)]" : "border-zinc-200 bg-white shadow-sm"}`}
            >
              <div
                className={`mb-4 flex flex-col gap-3 border-b pb-4 sm:flex-row sm:items-center sm:justify-between ${dark ? "border-white/10" : "border-zinc-200"}`}
              >
                <div>
                  <h2
                    className={`text-lg font-semibold ${dark ? "text-zinc-50" : "text-zinc-900"}`}
                  >
                    Your Youtube Videos
                  </h2>
                </div>
              ...
              {hiddenVideos.size > 0 && (
                <div className="mb-3 flex w-full justify-end">
                  ...
                  >
                    {showHidden
                      ? "Hide hidden videos"
                      : `Show hidden (${hiddenVideos.size})`}
                  </button>
                </div>
              )}
              <div
                className={`rounded-2xl border border-dashed p-4 ${dark ? "border-white/10 bg-zinc-950/30" : "border-zinc-200 bg-zinc-50"}`}
              >
                <div className="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {(showAllVideos
                    ...
                    : portfolioItems
                        .filter(
                          (item) =>
                            showHidden || !hiddenVideos.has(item.youtubeId),
                        )
                        .slice(0, 6)
                  )
                    .filter(
                      (item) => showHidden || !hiddenVideos.has(item.youtubeId),
                    )
                    .map((item) => (
                        ...
                        >
                          <div className="relative aspect-video overflow-hidden ">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                              <div
                                className={`flex h-12 w-12 items-center justify-center rounded-full text-sky-400 shadow-lg transition group-hover:scale-110 ${dark ? "bg-zinc-900/80 shadow-black/60 group-hover:bg-zinc-900" : "bg-white/90 shadow-black/20 group-hover:bg-white"}`}
                              >
                                <svg
                          ...
                          <div className="flex flex-1 flex-col gap-1 px-4 py-3">
                            <h3
                              className={`text-sm font-semibold ${dark ? "text-zinc-100" : "text-zinc-900"}`}
                            >
                              {item.title}
                            </h3>
                            <p
              ...
              {(() => {
                const visible = portfolioItems.filter(
                  (item) => !hiddenVideos.has(item.youtubeId),
                );
                return (
                  visible.length > 6 && (
                    <button
                    ...
                    >
                      {showAllVideos
                        ? "Show less"
                        : `Show more (${portfolioItems.filter((item) => !hiddenVideos.has(item.youtubeId)).length - 6} more)`}
                    </button>
                  )
                );
              })()}
            ...
            <div
              className={`flex flex-col rounded-3xl border p-6 backdrop-blur-sm ${dark ? "border-white/10 bg-zinc-900/40 shadow-[0_30px_120px_rgba(0,0,0,0.5)]" : "border-zinc-200 bg-white shadow-sm"}`}
            >
              <div
                className={`mb-4 flex items-center justify-between border-b pb-4 ${dark ? "border-white/10" : "border-zinc-200"}`}
              >
                <h2
                  className={`text-lg font-semibold ${dark ? "text-zinc-50" : "text-zinc-900"}`}
                >
              ...
              >
                ...
                <Button
                  ...
                  className="self-start bg-sky-600 text-white hover:bg-sky-500"
                >
            ...
            >
              ...
              >
                ...
                <span
                  className={`text-xs px-2 py-1 rounded-full ${dark ? "bg-emerald-500/20 text-emerald-400" : "bg-emerald-100 text-emerald-700"}`}
                >
              ...
              <div className="flex flex-col gap-3">
                {contactMessages.length === 0 && (
                  <p
                    className={`text-center text-sm ${dark ? "text-zinc-500" : "text-zinc-400"}`}
                  >
                    No messages yet.
                  </p>
                )}
                {contactMessages.map((m) => (
                  <div
                    key={m._id}
                    className={`relative rounded-2xl border p-4 ${dark ? "border-white/10 bg-zinc-950/50" : "border-zinc-200 bg-zinc-50"}`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-1">
                        ...
                        <p
                          className={`text-[10px] mt-1 ${dark ? "text-zinc-600" : "text-zinc-400"}`}
                        >
            ...
            >
              ...
              >
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setTestimonialForm((p) => ({ ...p, quote: e.target.value }))
                  }
                  required
                  className={
                    dark
                      ? "border-white/15 bg-zinc-950/60 text-zinc-100 placeholder:text-zinc-500"
                      : ""
                  }
                />
                <Button
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {testimonials.length === 0 && (
                  <p
                    className={`col-span-full text-center text-sm ${dark ? "text-zinc-500" : "text-zinc-400"}`}
                  >
                    No testimonials yet.
                  </p>
                )}
                {testimonials.map((t) => (
                  <div
                    key={t._id}
                    className={`relative flex flex-col gap-2 rounded-2xl border p-4 ${dark ? "border-white/10 bg-zinc-950/50" : "border-zinc-200 bg-zinc-50"}`}
                  >
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {ads.map((ad) => (
                  <div
                    key={ad._id ?? ad.src}
                    className={`relative flex flex-col gap-3 rounded-2xl border p-4 ${dark ? "border-white/10 bg-zinc-950/50" : "border-zinc-200 bg-zinc-50"}`}
                  >
                    <img
                      src={ad.src}
                      alt="Ad image"
                ...
                {galleryPhotos.map((photo) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        photo._id && handleDeleteGalleryPhoto(photo._id)
                      }
                      className="absolute right-3 top-3 text-red-400 hover:text-red-300"
